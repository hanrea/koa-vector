 ...
 */
Vector.prototype.next =  (path, domain) => {
    let next = new Vector();
...
/**
 * 下级无法覆盖上级设置的域名
 * Can't set domain filter if upper level had set 
 * 支持正则、字符串、通配符
 * support regexp、string、wildcard
 * @param {*} domain curent router domain
 */
Vector.prototype.setDomain = function (domain) {
    if (this.domain) {
        next.extDomain = true;
 ...
 */
Vector.prototype.allow =  (methods, ...cbs) => {
    let path = this.path;
    cbs.forEach( (cb) => {
        isGenerator(cb, path);
    });
 ...
 */
Vector.prototype.match =  (request, path) => {
    let cbs = null;
    let that = this;
    ...
    // 域名检测(域名不通过不用处理)
    if (!(that.domain === "" || that.domain === "*" || that.domain === _hostname || that.domainRegex.match(_hostname))) {
        return [];
    }
